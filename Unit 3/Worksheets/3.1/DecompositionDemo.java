public class DecompositionDemo {
    ...
    // Creates three arrays of hours, steps, and activeMinutes for a 12 hour period
    // Makes sure that all three arrays have same length
    // Calculate total steps -> calculate average steps
    // While for loop for total steps: adjust min and max
    // Prints report

    // calculate avgSteps, totalSteps, minSteps, maxSteps, maxStepsHour (5 things)
    public static double[] calculateStepStats(int n, int[] steps, int[] hours) {
        ...
        results[0] = avgSteps;
        results[1] = totalSteps;
        results[2] = minSteps;
        results[3] = maxSteps;
        results[4] = maxStepsHour;
        return results;
    }

    public static boolean isLengthsValid(int l1, int l2, int l3) {
        if (l1 == l2 && l2 == l3) {
            return true;
        }
        return false;
    ...
    public static void bigMethodToDecompose() {
        ...
        double[] results = calculateStepStats(hours.length, steps, hours);
        double avgSteps = results[0];
        int totalSteps = (int) results[0];
        int minSteps = (int) results[0];
        int maxSteps = (int) results[0];
        int maxStepsHour = (int) results[0];

        int n = hours.length;

        // Validate equal lengths
        if (! isLengthsValid(hours.length, steps.length, activeMinutes.length)) {
